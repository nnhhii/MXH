...
$sql = "SELECT user_id, username, bio, cover_picture,avartar FROM USER WHERE user_id =  {$_SESSION['user']}";
$result = $link->query($sql);
$row = $result->fetch_assoc();

$sql_post = "SELECT * FROM posts WHERE post_by =  {$_SESSION['user']}";
$kq = $link->query($sql_post);
?>
<style>
  body {
    margin: 0;
...
<body>
  ...
  <div class="post_TCN" style="margin:0 13%">
    <div class="central-meta" style="padding:25px">
      <ul class="photos">
        <?php while ($row_post = mysqli_fetch_assoc($kq)) { ?>
          <!-- P1 -->
          <li>
